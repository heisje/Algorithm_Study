...
def solution(n, info):
    ...
    def make_array(arrow, i, array, lion, peach, info):       # 라이언이 쏠 수 있는 과녁의 경우의 수를 모두 계산할 함수 (DFS)
        ...
        if i == 11 or arrow == 0:                             # 리스트를 다 돌았거나 남은 화살이 없을 때
            ...
            array[10] = 0                                     # 화살이 남았을 때 더해준 경우가 있으므로 0점 과녁을 0으로 만들어줌.
            return
    ...
    if array_dict and len(array_dict[max_diff]) > 1:          # 가장 큰 점수차인 array가 2개 이상일 때 array들을 reverse 후 sort
        array_dict[max_diff] = sorted(list(map(lambda x: list(reversed(x)), array_dict[max_diff])))
        return list(reversed(array_dict[max_diff][-1]))       # 가장 마지막에 있는 array가 가장 낮은 점수를 맞힌 경우가 마지막에 옴.
    return array_dict[max_diff][-1] if array_dict else [-1]


# 테스트 1 〉	통과 (0.04ms, 10.3MB)
# 테스트 2 〉	통과 (0.08ms, 10.2MB)
# 테스트 3 〉	통과 (0.12ms, 10.5MB)
